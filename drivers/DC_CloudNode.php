...
class CloudNodeDataContainer extends DC_CloudNode
{
	...
	public function showAll()
	{
		...
		if (\Input::get('tg') == 'all')
		{
			...
			// Expand tree
			$strKey = $this->objCloudApi->getName() . 'Filetree';
			if (!is_array($session[$strKey]) || empty($session[$strKey]) || current($session[$strKey]) != 1)
			{
				$session[$strKey] = $this->getMD5Folders($this->objCloudApi->getRoot());
			}
			// Collapse tree
			else
			{
				$session[$strKey] = array();
			}
		...
		{
			$return .= $this->generateTree($this->objCloudApi->getRoot(), 0, false, false, ($blnClipboard ? $arrClipboard : false));
		}
		else
		{
...
<div class="tl_submit_container">' . (!$GLOBALS['TL_DCA'][$this->strTable]['config']['notDeletable'] ? '
  ...
  <input type="submit" name="edit" id="edit" class="tl_submit" accesskey="s" value="'.specialchars($GLOBALS['TL_LANG']['MSC']['editSelected']).'"> ' : '') . $callbacks . '
	...
	public function create()
	{
		...
		{
			$this->log('Folder "'.$strFolder.'" was not mounted or is not a directory', 'DC_CloudNode create()', TL_ERROR);
		...
		$this->objCloudApi->getNode($strFolder . '/__new__');
		$this->objCloudApi->save();
	...
	public function cut($blnDoNotRedirect=false)
	{
		...
		if (!$this->objCloudApi->nodeExists($this->intId) || !$this->isMounted($this->intId))
		{
			$this->log('File or folder "'.$this->intId.'" was not mounted or could not be found', 'DC_CloudNode cut()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}

		if (!$this->objCloudApi->nodeExists($strFolder) || !$this->isMounted($strFolder))
		{
			$this->log('Parent folder "'.$strFolder.'" was not mounted or is not a directory', 'DC_CloudNode cut()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		...
		{
			$this->log('Attempt to move the folder "'.$this->intId.'" to "'.$strFolder.'" (circular reference)', 'DC_CloudNode cut()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		...
		$objNode = $this->objCloudApi->getNode($this->intId);
		$objNode->move($destination);

		// Find the corresponding DB entries
		if ($this->blnIsDbAssisted)
		{			
			$objFile = CloudNodesModel::findByPath($this->intId);

			// Set the parent ID
			if ($strFolder == $this->objCloudApi->getRoot())
			{
				$objFile->pid = 0;
			}
			else
			{
				$objFolder = CloudNodesModel::findByPath($strFolder);
			...
			{
				$objFiles = CloudNodesModel::findMultipleByBasepath($this->intId.'/');
			...
			{
				if ($strPath != $this->objCloudApi->getRoot())
				{
					$objModel = CloudNodesModel::findByPath($strPath);
					
					// create new folder node
					$objFolder = new $this->objCloudApi->getNode($objModel->path);
					$objFolder->save();
		...
		// Add a log entry
		$this->log('File or folder "'.$this->intId.'" has been moved to "'.$destination.'"', 'DC_CloudNode cut()', TL_FILES);
	...
	public function copy($source=null, $destination=null)
	{
		...
		if (!$this->objCloudApi->nodeExists($source) || !$this->isMounted($source))
		{
			$this->log('File or folder "'.$source.'" was not mounted or could not be found', 'DC_CloudNode copy()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}

		if (!$this->objCloudApi->nodeExists($strFolder) || !$this->isMounted($strFolder))
		{
			$this->log('Parent folder "'.$strFolder.'" was not mounted or is not a directory', 'DC_CloudNode copy()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		...
		{
			$this->log('Attempt to copy the folder "'.$source.'" to "'.$strFolder.'" (circular reference)', 'DC_CloudNode copy()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		...
		$objNode = $this->objCloudApi->getNode($source);
		...
		if ($objNode->type == 'folder')
		{
			...
			while ($this->objCloudApi->nodeExists($new) && $count < 12)
			{
			...
			if ($this->blnIsDbAssisted)
			{
				$objFolder = CloudNodesModel::findByPath($source);
			...
			{
				if ($objChild->name == '.svn' || $objChild->name == '.DS_Store')
				{
					continue;
				}
				...
				{
					$this->copy($source . '/' . $objChild->name, $destination . '/' . $objChild->name);
				}
				else
				{
					$objChild->copy($destination . '/' . $objChild->name);

					// Find the corresponding DB entries
					if ($this->blnIsDbAssisted)
					{
						$objFile = CloudNodesModel::findByPath($source . '/' . $file);
		...
		else
		{
			...
			$destination = $new;
			$objNode->copy($destination);		
			$strFolder = dirname($destination);

			// Find the corresponding DB entries
			if ($this->blnIsDbAssisted)
			{
				$objFile = CloudNodesModel::findByPath($source);
				$objNewFile = clone $objFile->current();
				...
				if ($strPath != $GLOBALS['TL_CONFIG']['uploadPath'])
				{
					...
					$objFolder = $this->objCloudApi->getNode($objModel->path);
					$objFolder->save();
		...
		if (!$noReload)
		{
			if ($objNode->nodeExists($source) && $this->isMounted($source))
			{
				$this->log('File or folder "'.$source.'" has been duplicated', 'DC_CloudNode copy()', TL_FILES);
			}
	...
	public function delete($source=null)
	{
		...
		if ($this->objCloudApi->nodeExists($source) || !$this->isMounted($source))
		{
			$this->log('File or folder "'.$source.'" was not mounted or could not be found', 'DC_CloudNode delete()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		...
		if ($objNode->type == 'folder')
		{
			...
			foreach ($arrChildren as $objChild)
			{
				...
				else
				{
					...
					// Find the corresponding DB entries
					if ($this->blnIsDbAssisted && $objChild->name != '.DS_Store')
					{
						$objFile = CloudNodesModel::findByPath($objChild->path);
						...
						{
							\Message::addError(sprintf($GLOBALS['TL_LANG']['ERR']['fileNotFoundSync'], $source . '/' . $objChild->name));
						}
					}
				}
			}
		...
		if (!$noReload)
		{
			$this->log('File or folder "'.str_replace(TL_ROOT.'/', '', $source).'" has been deleted', 'DC_CloudNode delete()', TL_FILES);
			$this->redirect($this->getReferer());
		}
	}
	...
	public function move($blnIsAjax=false)
	{
		...
		{
			$this->log('Folder "'.$strFolder.'" was not mounted or is not a directory', 'DC_CloudNode move()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}

		if (!preg_match('/^'.preg_quote($GLOBALS['TL_CONFIG']['uploadPath'], '/').'/i', $strFolder))
		{
			$this->log('Parent folder "'.$strFolder.'" is not within the files directory', 'DC_CloudNode move()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		...
		if (\Input::post('FORM_SUBMIT') == 'tl_upload')
		{
			...
			if ($this->blnIsDbAssisted)
			{
				...
				{
					$objFile = CloudNodesModel::findByPath($strFile);
...
<input type="submit" name="uploadNback" class="tl_submit" accesskey="c" value="'.specialchars($GLOBALS['TL_LANG'][$this->strTable]['uploadNback']).'">
	...
	public function edit()
	{
		...
		{
			$this->log('File or folder "'.$this->intId.'" was not mounted or could not be found', 'DC_CloudNode edit()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
...
window.addEvent(\'domready\', function() {
  Backend.vScrollTo(($(\'' . $this->strTable . '\').getElement(\'label.error\').getPosition().y - 20));
	...
	public function editAll()
	{
		...
		{
			$this->log('Table "'.$this->strTable.'" is not editable', 'DC_CloudNode editAll()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		...
		if (is_array($fields) && !empty($fields) && \Input::get('fields'))
		{
			...
			foreach ($ids as $id)
			{
				...
				if ($this->blnIsDbAssisted)
				{
					$objFile = CloudNodesModel::findByPath($id);
					$this->objActiveRecord = $objFile;
				}
...
<a href="'.$this->getReferer(true).'" class="header_back" title="'.specialchars($GLOBALS['TL_LANG']['MSC']['backBTTitle']).'" accesskey="b" onclick="Backend.getScrollOffset()">'.$GLOBALS['TL_LANG']['MSC']['backBT'].'</a>
	...
	public function source()
	{
		...
		if (is_dir(TL_ROOT .'/'. $this->intId))
		{
			$this->log('Folder "'.$this->intId.'" cannot be edited', 'DC_CloudNode source()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		elseif (!file_exists(TL_ROOT .'/'. $this->intId))
		{
			$this->log('File "'.$this->intId.'" does not exist', 'DC_CloudNode source()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		...
		{
			$this->log('Not enough permissions to edit the file source of file "'.$this->intId.'"', 'DC_CloudNode source()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		...
		{
			$this->log('File type "'.$objFile->extension.'" ('.$this->intId.') is not allowed to be edited', 'DC_CloudNode source()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		...
		if (\Input::post('FORM_SUBMIT') == 'tl_files')
		{
			...
			if (md5($strContent) != md5(\Input::postRaw('source')))
			{
				...
				if ($this->blnIsDbAssisted)
				{
					$objMeta = CloudNodesModel::findByPath($objFile->value);
...
<input type="submit" name="saveNclose" id="saveNclose" class="tl_submit" accesskey="c" value="'.specialchars($GLOBALS['TL_LANG']['MSC']['saveNclose']).'">
	...
	public function protect()
	{
		...
		if (file_exists(TL_ROOT . '/' . $this->intId . '/.htaccess'))
		{
			...
			$objFolder->unprotect();
			$this->log('The protection from folder "'.$this->intId.'" has been removed', 'DC_CloudNode protect()', TL_FILES);
			$this->redirect($this->getReferer());
		}
		// Protect the folder
		else
		{
			$objFolder = new \Folder($this->intId);
			$objFolder->protect();
			$this->log('Folder "'.$this->intId.'" has been protected', 'DC_CloudNode protect()', TL_FILES);
			$this->redirect($this->getReferer());
		}
	}
	...
	protected function save($varValue)
	{
		...
		if ($this->strField == 'name')
		{
			...
			if ($this->blnIsDbAssisted)
			{
				...
				{
					$objFolder = CloudNodesModel::findByPath($this->strPath);
					$pid = $objFolder->id;
				}
				...
				if (stristr($this->intId, '__new__') == true)
				{
					...
					// Add a log entry
					$this->log('Folder "'.$this->strPath.'/'.$varValue.$this->strExtension.'" has been created', 'DC_CloudNode save()', TL_FILES);
				}
				else
				{
					// Find the corresponding DB entry
					$objFile = CloudNodesModel::findByPath($this->strPath . '/' . $this->varValue . $this->strExtension);
					...
					// Add a log entry
					$this->log('File or folder "'.$this->strPath.'/'.$this->varValue.$this->strExtension.'" has been renamed to "'.$this->strPath.'/'.$varValue.$this->strExtension.'"', 'DC_CloudNode save()', TL_FILES);
				}
				...
				if ($objFile->type == 'folder')
				{
					...
					$objFiles = CloudNodesModel::findMultipleByBasepath($strPath);
				...
				if ($objFile->pid > 0)
				{
					$objModel = CloudNodesModel::findByPk($objFile->pid);
					$objFolder = new \Folder($objModel->path);
					$objModel->hash = $objFolder->hash;
					$objModel->save();
				}
...
';
	...
	protected function isValid($strFile)
	{
		...
		{
			$this->log('Invalid file name "'.$strFile.'" (hacking attempt)', 'DC_CloudNode isValid()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		elseif (strpos($strFolder, '../') !== false)
		{
			$this->log('Invalid folder name "'.$strFolder.'" (hacking attempt)', 'DC_CloudNode isValid()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		...
		if (!empty($this->arrValidFileTypes) && is_file(TL_ROOT . '/' . $strFile))
		{
			...
			{
				$this->log('File "'.$strFile.'" is not an allowed file type', 'DC_CloudNode isValid()', TL_ERROR);
				$this->redirect('contao/main.php?act=error');
			}
		}
		...
		{
			$this->log('File or folder "'.$strFile.'" is not within the files directory', 'DC_CloudNode isValid()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		...
		{
			$this->log('Parent folder "'.$strFolder.'" is not within the files directory', 'DC_CloudNode isValid()', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		...
		if (\Input::get('act') == 'edit' || \Input::get('act') == 'paste' || \Input::get('act') == 'delete')
		{
			...
			{
				$this->log('Attempt to edit, copy, move or delete the root folder "'.$strFile.'"', 'DC_CloudNode isValid()', TL_ERROR);
				$this->redirect('contao/main.php?act=error');
			}
		}
